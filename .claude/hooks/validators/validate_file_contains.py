...
def validate_file_contains(
    ...
    logger.info(
        f"Validating: directory={directory}, extension={extension}, max_age={
            max_age_minutes
        }min"
    )
    ...
    if all_found:
        msg = f"File '{newest_file}' contains all {
            len(required_strings)
        } required sections"
        logger.info(f"PASS: {msg}")
        return True, msg
    else:
...
def main():
    ...
    try:
        ...
        args = parse_args()
        logger.info(
            f"Args: directory={args.directory}, extension={args.extension}, max_age={
                args.max_age
            }"
        )
